...
router.post("/wallets", async (req, res) => {
    try {
        const { userId, name, currency, icon, type } = req.body
        if (!userId || !name) {
...
router.put("/accept-invite", async (req, res) => {
    try {
        ...
        const invite = await InviteToWallet.findById(inviteId)
            .populate("wallet")
            .populate("user")
...
router.post("/movements", async (req, res) => {
    try {
        ...
        if (!wallet || !user || !type || !amount || !category) {
            return res.status(400).json({ error: "Faltan datos obligatorios" })
        }

        const movement = await SpendingMovement.create({
            wallet,
            user,
            type,
            amount,
